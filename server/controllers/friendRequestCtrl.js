...
const friendRequestCtl = {
  ...
  sendRequest: async (req, res) => {
    try {
      ...
      if (!idUser || !user)
        return res.status(403).json({ msg: "User send not found" });
      ...
      if (oldRequest)
        return res.status(400).json({ msg: "Request has been create" });
      ...
      return res
        .status(200)
        .json({ msg: `Sent request add friend to ${receiverUser.username}` });
    } catch (err) {
      return res.status(500).json({ msg: err.message });
    }
  },
  ...
  acceptRequest: async (req, res) => {
    try {
      ...
      if (!request)
        return res.status(400).json({ msg: "This quest does not exit" });
